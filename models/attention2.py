...
class CrossAttention(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=4, dim_head=512, dropout=0.):
        super().__init__()
        inner_dim = dim_head * heads
        context_dim = default(context_dim, query_dim)
        self.pe_dim = 22
        self.feat_embedding = nn.Sequential(
            nn.Linear(2, context_dim - self.pe_dim),
        )
        self.before_mean = nn.Sequential(
            nn.Linear(context_dim, query_dim),
        )
        self.mean_pool = nn.AdaptiveAvgPool2d((83, query_dim))
        ...
        # self.norm1 = nn.BatchNorm1d(1660)
        # self.norm1 = nn.LayerNorm(22)
    ...
    def forward(self, x, context=None, mask=None):
        ...
        if context != None:
            emb_val = self.feat_embedding(context[..., self.pe_dim :])

            context = torch.cat([context[..., :self.pe_dim], emb_val], dim=-1)
            context = self.before_mean(context)
            context = self.mean_pool(context)
...
class SpatialTransformer(nn.Module):
    ...
    """
    def __init__(self, in_channels, n_heads, d_head = 512,
